# Chay den het synthesis roi DUNG — khong sang implementation, khong bitstream.
#
# Vi sao can: tcl/run.tcl chay thang mot mach synth -> impl -> bitstream, mat
# nhieu gio. Repo nay CHUA TUNG chay synthesis lan nao, nen lan dau can bat loi
# som: loi ngon ngu, thieu file, sai ten cell trong XDC deu lo ra o synth.
#
# LUU Y: synth chay duoc KHONG chung minh XDC da dung. Cong khong duoc rang
# buoc chan thuong chi no o DRC luc implementation. Muon kiem tra day du thi
# chay tcl/run.tcl.
#
# Dung: make -C fpga/pulp synth BOARD=zcu104
...
if {[get_property PROGRESS [get_runs synth_1]] != "100%"} {
    puts "SYNTH-ONLY: synthesis THAT BAI"
    exit 1
}
...
report_utilization       -file reports/${PROJECT}_synth_utilization.rpt
report_utilization       -hierarchical -file reports/${PROJECT}_synth_utilization_hier.rpt
report_timing_summary    -file reports/${PROJECT}_synth_timing_summary.rpt
report_clocks            -file reports/${PROJECT}_synth_clocks.rpt
report_drc -ruledeck default -file reports/${PROJECT}_synth_drc.rpt

puts "SYNTH-ONLY: xong. Bao cao o fpga/pulp/reports/"
exit 0
