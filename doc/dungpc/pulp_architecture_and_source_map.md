...
Trong wiring hiện tại, chỉ clock/reset được đưa vào `cluster_domain_i`. Hai output fetch-enable và boot-address được tạo bởi SoC nhưng không phải cổng của wrapper Cluster. Bên trong `cluster_domain`, `fetch_en_i` bị buộc `0` và `base_addr_i` bị buộc `'0`. Vì vậy không thể kết luận từ hai output này rằng core Cluster đã bắt đầu fetch; cơ chế khởi chạy thực tế bên trong đúng revision dependency cần được xác minh sau checkout. Cluster vẫn báo trạng thái bận và trao đổi event/AXI với SoC.
...
FC cấu hình boot address và trạng thái Cluster
        |
        v
SoC cấp clock/reset; software điều khiển Cluster qua path cần xác minh trong dependency
        |
        v
